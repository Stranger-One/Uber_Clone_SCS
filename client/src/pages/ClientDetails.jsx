...
const ClientDetails = () => {
  ...
  return (
    <section className="w-full h-screen relative">
      <div className="header flex items-center justify-between p-4 fixed top-0 left-0 w-full z-10 bg-white">
        <button type="button" onClick={() => navigate(-1)} className="">
          <FaArrowLeft size={32} />
        </button>
        <h2 className="text-xl font-semibold">#{newRideDetails[0]?.user.socketId?.slice(0, 10)}</h2>
      </div>
      ...
      <div className="w-full p-2 flex flex-col justify-between h-full pt-20">
        <div className="flex items-start justify-between py-2">
          <div className=" relative flex items-start gap-2">
            <div
              className="w-14 h-14 bg-gray-500 rounded-lg"
              style={{
                backgroundImage: "url('')",
              }}
            ></div>
            <h2 className="text-2xl font-semibold capitalize">{`${newRideDetails[0]?.user?.fullname?.firstname} ${newRideDetails[0]?.user?.fullname?.lastname}`}</h2>
          </div>
